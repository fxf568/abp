...
{
    public class MongoClientRepository : MongoDbRepository<ISkuoIdentityServerMongoDbContext, Client, long>, IClientRepository
    {
        public MongoClientRepository(
            IMongoDbContextProvider<ISkuoIdentityServerMongoDbContext> dbContextProvider
            ) : base(
                dbContextProvider)
        {
        }
        ...
        public virtual async Task<bool> CheckClientIdExistAsync(string clientId, long? expectedId = null, CancellationToken cancellationToken = default)
        {

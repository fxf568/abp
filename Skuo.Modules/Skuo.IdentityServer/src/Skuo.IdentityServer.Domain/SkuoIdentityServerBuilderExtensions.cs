...
using Skuo.Identity;
using Skuo.IdentityServer.AspNetIdentity;
using Volo.Abp.Security.Claims;

namespace Skuo.IdentityServer
{
    public static class SkuoIdentityServerBuilderExtensions
    {
        public static IIdentityServerBuilder AddAbpIdentityServer(
            this IIdentityServerBuilder builder,
            SkuoIdentityServerBuilderOptions options = null)
        {
            if (options == null)
            {
                options = new SkuoIdentityServerBuilderOptions();
            }
            ...
            if (options.IntegrateToAspNetIdentity)
            {
                ...
                builder.AddProfileService<SkuoProfileService>();
                builder.AddResourceOwnerValidator<SkuoResourceOwnerPasswordValidator>();
            }

            builder.Services.Replace(ServiceDescriptor.Transient<IClaimsService, SkuoClaimsService>());

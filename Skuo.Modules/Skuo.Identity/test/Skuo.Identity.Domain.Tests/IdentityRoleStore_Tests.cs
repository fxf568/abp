...
{
    public class IdentityRoleStore_Tests : SkuoIdentityDomainTestBase
    {
        ...
        public async Task AddClaimAsync()
        {
            try
            {
                using (var uow = _unitOfWorkManager.Begin())
                {
                    ...
                    role.Claims.Add(new IdentityRoleClaim(_idGenerator.NextId(), role.Id, "my-claim", "my-value", role.TenantId));
